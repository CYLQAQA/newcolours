// ---------- Helpers ----------
    ...
    const sheetNameToRegionMap = {
      ...
      'DUBLIN':'DUB',
      'CANADA':'CA',
      'GERMANY':'IRE'   // DE bucket into IRE
    };
    function mapSheetNameToRegion(sheetName){
      ...
      const known=['AU','CA','US','UK','IRE','JP','HK','DUB'];
    ...
    document.getElementById('refInput').addEventListener('change',async (e)=>{
      ...
      try{ await loadReference(f); alert('Reference map loaded.'); }
      catch(err){ alert('Failed to load reference: '+err.message); }
    });

    document.getElementById('nameMapInput').addEventListener('change', async (e) => {
      const f=e.target.files && e.target.files[0];
      if(!f) return;
      try{ await loadNameMapping(f); alert('Name mapping loaded.'); }
      catch(err){ alert('Failed to load name mapping: '+err.message); }
    });
