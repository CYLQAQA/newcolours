...
function renderCompanyPdf(doc, company, cc, cp, currLabel, prevLabel) {
  ...
  // ----- KPI bubbles (match dashboard: label + value + inline colored delta) -----
  const kpiY = margin + 50;
  const kpiH = 72;
  const kpiW = (pageW - margin * 2 - 12) / 4;
  const wq = qtyByCond(cc,'Working'), dq = qtyByCond(cc,'Damaged');
  const wqPrev = qtyByCond(cp,'Working'), dqPrev = qtyByCond(cp,'Damaged');
  const kpis = [
    { label: 'Total Qty',    value: fmtNum(cq), delta: cq - pq,     prev: pq,     money: false },
    { label: 'Working Qty',  value: fmtNum(wq), delta: wq - wqPrev,  prev: wqPrev, money: false },
    { label: 'Damaged Qty',  value: fmtNum(dq), delta: dq - dqPrev,  prev: dqPrev, money: false },
    { label: 'Total USD',    value: fmtUSD(cu), delta: cu - pu,      prev: pu,     money: true },
  ];
  const COLOR_GREEN  = [22, 163, 74];   // --green #16a34a
  const COLOR_RED    = [220, 38, 38];   // --red   #dc2626
  const COLOR_MUTED  = [107, 114, 128]; // --muted #6b7280
  const COLOR_LABEL  = [110, 110, 110];
  const COLOR_VALUE  = [31, 78, 120];
  const COLOR_BORDER = [200, 200, 200];
  const COLOR_BG     = [250, 251, 252]; // #fafbfc
  ...
  kpis.forEach((k, i) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(String(k.value), x + 8, kpiY + 36);
    // Delta line: arrow + abs(delta) + pct, colored
    const pct = momPct(k.delta, k.prev);
    const cls = k.delta > 0 ? 'up' : (k.delta < 0 ? 'down' : 'flat');
    const arrow = k.delta > 0 ? '▲' : (k.delta < 0 ? '▼' : '→');
    const deltaStr = k.money ? fmtUSD(Math.abs(k.delta)) : fmtNum(Math.abs(Math.round(k.delta)));
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...(cls === 'up' ? COLOR_GREEN : cls === 'down' ? COLOR_RED : COLOR_MUTED));
    doc.text(`${arrow} ${deltaStr} (${fmtPct(pct)})`, x + 8, kpiY + 56);
  });
  ...
  // Optional 5th+ args: opts = { x, width } — overrides default full-width placement
  function drawTable(startY, title, columns, rows, opts) {
    const tableX = (opts && opts.x != null) ? opts.x : margin;
    const tableW = (opts && opts.width != null) ? opts.width : (pageW - margin * 2);
    // Page-break guard: if the table would start below the page bottom, push to next page first
    let y = startY;
    const titleH = 14, headerH = 18, rowH = 14;
    const neededH = titleH + headerH + rows.length * rowH + 6;
    ...
    // outer border (using tracked tableTop so it's correct even after a page break)
    doc.setDrawColor(120, 120, 120);
    doc.rect(tableX, tableTop + titleH, tableW, y - (tableTop + titleH), 'S');
    return y + 6;
  }

  // ----- Country tables (continue on page 1, below charts) -----
  const ccG = groupBy(cc, r => r.Country || '(blank)');
  const cpG = groupBy(cp, r => r.Country || '(blank)');
  const cKeys = [...new Set([...ccG.keys(), ...cpG.keys()])].sort();

  let y = chartsY + chartH + 16;
  // Country current
  ...
  // ----- Model-grade tables (page 2, side by side) -----
  doc.addPage();
  const mgField = FILTERS.mgMode === 'model' ? 'ModelName' : 'ModelGrade';
  const mgLabel = FILTERS.mgMode === 'model' ? 'Model' : 'Model Grade';
  ...
  drawTable(margin, `Model-Grade — ${prevLabel} (${mgLabel})`, [
    { label: mgLabel, numeric: false },
    { label: 'Quantity', numeric: true },
    { label: 'Total USD', numeric: true, fmt: '$' },
  ], eData, { x: margin + halfW + 12, width: halfW });
}
...
function buildCompanyWorkbook(company, cc, cp, currLabel, prevLabel) {
  ...
  const aRows = [...G_curr.keys()].sort().map(k=>{
    ...
    const parts = k.split('||');
    parts[3] = fmtYYMMDD(parts[3]); // CreatedAt → YYMMDD
  ...
  // ---------- Block B: Country breakdown (current) — to the RIGHT of A, same starting row ----------
  // A occupies cols 0..8 (9 cols). B starts at row 0, col 10 (1-col gap at col 9).
  const bColStart = 10;
  ...
  putAOA(ws, [bHeader, ...bRowsWithTotal], 0, bColStart);
  const bRowEnd = bRowsWithTotal.length; // header + data + total, starting at row 0

  // ---------- Block C: Country MoM — 2 rows BELOW B, same column 10 ----------
  const cRowStart = bRowEnd + 2; // 2-row gap below B (B occupies rows 0..bRowEnd-1)
  ...
  putAOA(ws, [cHeader, ...cRows], cRowStart, bColStart);
  const cRowEnd = cRowStart + cRows.length; // header + data + total

  // ---------- Blocks D and E: Model-grade summaries, below everything, side by side ----------
  const deStart = Math.max(aEnd, cRowEnd) + 3; // 3-row gap below the larger of A / C
  ...
  const lastDataRow = Math.max(eHeaderRow + Math.max(dData.length, eData.length), cRowEnd - 1);
  const lastCol = Math.max(8, bColStart + cHeader.length - 1); // 18 (col 0..18)
  ws['!ref'] = XLSX.utils.encode_range({s:{r:0,c:0}, e:{r:lastDataRow, c:lastCol}});
  ws['!cols'] = [
    {wch:16}, {wch:12}, {wch:14}, {wch:10}, {wch:14}, {wch:12}, {wch:12}, {wch:12}, {wch:14}, // cols 0..8 (Block A)
    {wch:4},                                                                                   // col 9 (gap)
    {wch:16}, {wch:12}, {wch:14}, {wch:10},                                                   // cols 10..13 (Block B)
    {wch:14}, {wch:12}, {wch:12}, {wch:14}, {wch:12}                                          // cols 14..18 (Block C tail)
  ];

  // ---------- Number formats ----------
  // Block A: rows 1..aRowsWithTotal.length, cols 5..8
  ...
  // Block B: rows 1..bRowsWithTotal.length, cols 11..13 (Quantity, Total$, % of total)
  for (let r=1; r<=bRowsWithTotal.length; r++) {
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:11}), '#,##0');
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:12}), '"$"#,##0.00');
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:13}), '0.00%');
  }
  // Block C: rows cRowStart+1..cRowStart+cRows.length, cols 11..18
  for (let r=cRowStart+1; r<=cRowStart+cRows.length; r++) {
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:11}), '#,##0');   // Qty (Prev)
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:12}), '"$"#,##0.00'); // $ (Prev)
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:13}), '#,##0');   // Qty (Curr)
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:14}), '"$"#,##0.00'); // $ (Curr)
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:15}), '#,##0');   // Qty Δ
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:16}), '0.0%');    // Qty %
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:17}), '"$"#,##0.00'); // $ Δ
    setCellNumFmt(ws, XLSX.utils.encode_cell({r:r,c:18}), '0.0%');    // $ %
  }
  // Block D: cols 1..2
  ...
  // Block A: header(row 0) + data + total, cols 0..8
  applyTableBorder(ws, 0, 0, aRowsWithTotal.length, 8);
  // Block B: header(row 0) + data + total, cols 10..13
  applyTableBorder(ws, 0, 10, bRowsWithTotal.length, 13);
  // Block C: header(cRowStart) + data + total, cols 10..18
  applyTableBorder(ws, cRowStart, 10, cRowStart+cRows.length, 18);
...
// Format a Date or string as YYMMDD (e.g. 2026-07-01 → "260701")
